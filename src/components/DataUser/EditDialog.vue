<template>
<div>
  <div class="text-bold text-body1">Edit User</div>
...
const { showLoading,hideLoading,putData,successNotif,errorNotif } = generalMethods()
const { formData,optionLevel,teamleader,
    supervisor,
    namesupervisor,
    ...
    .then(valid=>{
      if(valid){
        if(formData.value.level.includes('sales')){
          ...
          showLoading()
          putData(`user/${props.dataDialog.id}`,formData.value)
          .then(res=>{
            emit('onAdd',res)
          })
          .catch(err=>{
...
onMounted(()=>{
  if(props.dataDialog){
    formData.value = {...props.dataDialog}
  }
    teamleader.value = '4'
})
</script>